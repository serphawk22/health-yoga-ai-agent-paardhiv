                    ...
                    <div className="space-y-6">
                        ...
                        >
                            <div className="p-5 md:p-8 flex flex-col items-center text-center">
                                ...
                                <p className="mt-8 text-[0.8125rem] text-zinc-500 font-medium uppercase tracking-widest opacity-40">June 21st • Global Celebration</p>
                            </div>
                        </motion.div>
                            ...
                            <div>
                                ...
                                {appointments.length > 0 ? (
                                                ...
                                                <div key={apt.id} className="flex items-start gap-4 pb-6 border-b border-white/5 last:border-0 last:pb-0">
                                                    ...
                                                    <div className="flex-1 min-w-0 pt-0.5">
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <h5 className="font-medium text-[0.8125rem] text-white/90 truncate tracking-tight">Dr. {apt.doctorName}</h5>
                                                            <span className={cn("text-[0.8125rem] font-medium uppercase px-2 py-0.5 rounded-full border tracking-[0.1em]", statusColors[apt.status] || "bg-zinc-500/20 text-zinc-300 border-zinc-500/30")}>
                                                                {apt.status}
                                                            </span>
                                                        </div>
                                                        <p className="text-[0.8125rem] text-zinc-500 font-light truncate">{apt.doctorSpecialization} • {typeLabels[apt.type] || apt.type}</p>
                                                        <div className="flex items-center gap-3 mt-1.5">
                                                            <div className="flex items-center gap-1.5 text-[0.8125rem] text-zinc-500 font-light">
                                                                <Clock className="w-3.5 h-3.5 opacity-60" /> {apt.scheduledTime}
                                                            </div>
                                                            {apt.meetingId && (
                                                                <Link href={`/appointments/call/${apt.meetingId}`} className="flex items-center gap-1.5 text-[0.8125rem] text-primary-400 hover:text-primary-300 transition-colors">
                                                                    <Video className="w-3.5 h-3.5" /> Join Call
                                                                </Link>
                                                            )}
                                                        </div>
                                                        {apt.reason && (
                                                            <p className="text-[0.8125rem] text-zinc-600 mt-1.5 truncate font-light">Reason: {apt.reason}</p>
                                                        )}
                                                    </div>
                                                </div>
                                            );
                                    ...
                                    <div className="flex flex-col items-center justify-center py-8 text-center">
                                        ...
                                        <p className="text-[0.8125rem] text-zinc-400 mb-1 font-medium">No upcoming appointments</p>
                                        <p className="text-[0.8125rem] text-zinc-600 font-light">Book a consultation to get started</p>
                                    </div>
                                )}
                            </div>
                            <div className="mt-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
                  <Link href="/plan">
                    ...
                    >
                      ...
                      <h3 className="text-[0.8125rem] font-medium text-health-text mb-1">View My Current Plan</h3>
                      <p className="text-[0.8125rem] text-zinc-500 font-light">Track daily adherence and view weekly progress</p>
                    </motion.div>
                  </Link>
                  ...
                  <Link href="/nutritionists">
                    ...
                    >
                      ...
                      <h3 className="text-[0.8125rem] font-medium text-health-text mb-1">Our Nutritionists</h3>
                      <p className="text-[0.8125rem] text-zinc-500 font-light">Meet certified health and nutrition experts</p>
                    </motion.div>
                  </Link>
                </div>
            </div>
...
function BenefitItem({ text }: { text: string }) {
    return (
        <li className="text-[0.8125rem] text-zinc-300">
            {text}
        </li>
    )
}
