  ...
  return (
    <div className="max-w-5xl mx-auto pb-20 lg:pb-6 relative z-10 text-zinc-100 min-h-screen">
      ...
      <div className="mb-8 no-print">
        <h1 className="text-3xl font-normal tracking-tight text-white mb-1">Diet Plan</h1>
        <p className="text-zinc-400 font-light">Get personalized nutrition recommendations</p>
      </div>

      {/* Tabs */}
      <div className="flex bg-zinc-950/40 backdrop-blur-md p-1.5 rounded-2xl border border-white/5 mb-8 no-print w-fit shadow-xl gap-1">
        <button
      ...
      {activeTab === 'generate' && (
        <div className="animate-fadeIn space-y-6">
          {/* Request Section */}
          <div className="bg-zinc-950/40 backdrop-blur-3xl border border-white/5 rounded-3xl p-8 shadow-2xl no-print relative overflow-hidden group">
            <div className="absolute inset-0 bg-gradient-to-br from-white/[0.02] to-transparent pointer-events-none" />
            <h2 className="font-medium text-white text-xl mb-1 relative z-10">Generate New Plan</h2>
          ...
          {dietPlan ? (
            <div id="printable-content" className="space-y-6">
              ...
              {/* Overview */}
              <div className="bg-zinc-950/40 backdrop-blur-3xl border border-white/5 rounded-3xl p-8 shadow-2xl print:shadow-none print:border-gray-200">
                <p className="text-[11px] font-medium text-zinc-400 uppercase tracking-widest mb-4">Daily Overview</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="p-4 rounded-2xl bg-orange-500/5 border border-orange-500/10 print:bg-gray-50 print:border-gray-200">
                    <p className="text-[11px] font-medium text-orange-400/80 uppercase tracking-widest mb-2 print:text-black">Daily Calories</p>
                    <p className="text-2xl font-semibold text-white print:text-black">{dietPlan.dailyCalories}</p>
                    <p className="text-[10px] text-zinc-400 mt-0.5">kcal</p>
                  </div>
                  {dietPlan.macros && (
                    <>
                      <div className="p-4 rounded-2xl bg-blue-500/5 border border-blue-500/10 print:bg-gray-50 print:border-gray-200">
                        <p className="text-[11px] font-medium text-blue-400/80 uppercase tracking-widest mb-2 print:text-black">Protein</p>
                        <p className="text-2xl font-semibold text-white print:text-black">{dietPlan.macros.protein}g</p>
                        <p className="text-[10px] text-zinc-400 mt-0.5">{Math.round((dietPlan.macros.protein * 4 / dietPlan.dailyCalories) * 100)}%</p>
                      </div>
                      <div className="p-4 rounded-2xl bg-green-500/5 border border-green-500/10 print:bg-gray-50 print:border-gray-200">
                        <p className="text-[11px] font-medium text-green-400/80 uppercase tracking-widest mb-2 print:text-black">Carbs</p>
                        <p className="text-2xl font-semibold text-white print:text-black">{dietPlan.macros.carbs}g</p>
                        <p className="text-[10px] text-zinc-400 mt-0.5">{Math.round((dietPlan.macros.carbs * 4 / dietPlan.dailyCalories) * 100)}%</p>
                      </div>
                      <div className="p-4 rounded-2xl bg-purple-500/5 border border-purple-500/10 print:bg-gray-50 print:border-gray-200">
                        <p className="text-[11px] font-medium text-purple-400/80 uppercase tracking-widest mb-2 print:text-black">Fats</p>
                        <p className="text-2xl font-semibold text-white print:text-black">{dietPlan.macros.fats}g</p>
                        <p className="text-[10px] text-zinc-400 mt-0.5">{Math.round((dietPlan.macros.fats * 9 / dietPlan.dailyCalories) * 100)}%</p>
                      </div>
                    </>
                  )}
                </div>
              ...
              {/* Meals */}
              <div className="bg-zinc-950/40 backdrop-blur-3xl border border-white/5 rounded-3xl p-8 shadow-2xl print:shadow-none print:border-gray-200">
                <p className="text-[11px] font-medium text-zinc-400 uppercase tracking-widest mb-4">Daily Meals</p>
              ...
              <div className="grid md:grid-cols-2 gap-4 print:block print:space-y-6">
                {dietPlan.foodsToInclude && (
                  <div className="bg-zinc-950/40 backdrop-blur-3xl border border-white/5 rounded-3xl p-8 shadow-2xl print:shadow-none print:border-gray-200 print:break-inside-avoid">
                ...
                <div className="bg-zinc-950/40 backdrop-blur-3xl border border-white/5 rounded-3xl p-8 shadow-2xl print:shadow-none print:border-gray-200 print:break-inside-avoid">
                  ...
                  <ul className="space-y-2">
                    {dietPlan.hydrationTips.map((tip: string, i: number) => (
                      <li key={i} className="text-sm text-zinc-200">• {tip}</li>
                    ))}
                  </ul>
                </div>
              )}
              ...
              {dietPlan.specialNotes && dietPlan.specialNotes.length > 0 && (
                <div className="bg-amber-500/5 border border-amber-500/20 rounded-3xl p-6 print:bg-transparent print:border-gray-200 print:break-inside-avoid">
                  <p className="text-[11px] font-semibold text-amber-400 uppercase tracking-widest mb-3">Special Notes</p>
                  <ul className="space-y-2">
              ...
              {/* Disclaimer */}
              <div className="p-5 rounded-2xl border border-white/10 text-sm text-zinc-400 print:bg-transparent print:text-xs print:mt-8">
                <strong className="text-zinc-300">Disclaimer:</strong> This diet plan is for general wellness purposes only.
                It is not a substitute for professional nutritional advice.
              </div>
            </div>
          ) : (
            <div className="text-center py-16 text-zinc-400">
              <p className="text-lg font-normal mb-1">No plan generated yet</p>
            ...
            <div className="space-y-3">
              {historyItems.map((item) => (
                <button
                  ...
                  className="w-full p-6 rounded-3xl bg-zinc-950/40 backdrop-blur-3xl border border-white/5 shadow-lg hover:bg-white/[0.05] hover:border-white/10 hover:scale-[1.01] transition-all text-left group"
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-normal text-white group-hover:text-primary-400 transition-colors">
                        {item.title}
                      </h3>
                      <div className="flex items-center gap-4 mt-1 text-sm font-extralight text-zinc-400">
                    ...
                    <span className="text-zinc-500 group-hover:text-zinc-300 transition-colors text-lg font-extralight">&rarr;</span>
                  </div>
                </button>
              ))}
            </div>
...
function MealCard({ meal }: { meal: any }) {
  ...
  const colorClass = mealColors[mealType] || 'text-zinc-300 border-white/10';

  return (
    <div className={cn("p-4 rounded-2xl border border-white/5 print:border-gray-200 print:break-inside-avoid", colorClass.split(' ').slice(1).join(' '))}>
      <div className="flex items-center justify-between mb-3">
        <div>
          <h4 className={cn("font-medium capitalize text-sm", colorClass.split(' ')[0])}>{meal.name}</h4>
          {meal.time && <p className="text-[11px] font-extralight text-zinc-400 mt-0.5">{meal.time}</p>}
        </div>
        {meal.calories && (
          <span className="text-xs font-medium text-zinc-400 bg-white/5 px-2.5 py-1 rounded-lg">{meal.calories} kcal</span>
        )}
      </div>
      ...
      {meal.items && (
        <ul className="space-y-1">
          {meal.items.map((item: string, i: number) => (
            <li key={i} className="text-sm text-zinc-300 flex items-start gap-2 print:text-black">
